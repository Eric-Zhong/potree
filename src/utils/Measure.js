...
export class Measure extends THREE.Object3D {
	...
	addMarker(point) {
		...
		// TODO: xuzhong 将创建 measuement (360 images) 时的 uuid 复制过来
		sphere.name = this.uuid;
		sphere.data = this.data;
		this.add(sphere);
		...
		{ // Event Listeners
			...
			// TODO: xuzhong 自定义添加 click 事件
			let mouseup = (e) => {
				// console.log('click 了', e);
				const sphere = e.target; // 球

				// 如果点击了目标图片标记
				if (sphere && sphere.data) {
					const event = new CustomEvent('onClickImagePoint', {
						detail: sphere.data,
					});
					...
					try {
						...
						console.log(error);
						// 跨域时
						let param = {...sphere.data};
						param.action = 'onClickImagePoint';
			...
			sphere.addEventListener('mouseup', mouseup);
		}
